# Development to Staging Promotion Reference

This reference contains project-agnostic command templates for opening, reusing, and inspecting `development` → `staging` promotion PRs. Replace placeholders with project-supplied values.
...
Use the default `development` and `staging` branch names unless the project explicitly documents a different ladder. Stop before mutations if either required branch is missing, ambiguous, or unexpectedly protected.
...
When reporting a PR handoff, tell the user that any provider branch-deletion prompt should be ignored for these source and target branches because they are supposed to remain available for future promotions.
...
What this is: a GitHub review page for the `development → staging` promotion.
What to do: open it, read the description, inspect Files changed, check the Checks tab, and read comments.
Boundary: Does not merge or deploy; it prepares an understandable handoff.
Branch precaution: do not delete the `development` or `staging` branches if GitHub offers branch cleanup; these are permanent promotion branches.
...
Infer the PR title and body before creating anything. Use the default lane-derived title unless project docs define an exact metadata contract. Build the body from verified branch, diff, topology, release-surface, review/check, and handoff evidence:
...
Do not edit, close or recreate mismatched PRs unless the user authorizes that mutation. If authorized, use the project-approved close/recreate process, for example:
...
| Conflicting PR | Source and target diverged | Report blocker; do not push target directly |
| Failing checks | CI or policy failure | List check names and URLs |
| Changes requested | Review blocker | Report review state |
| Missing branch | Remote branch absent | Stop and ask for branch policy |
...
5. Failing or pending checks.
6. Conflicts, branch protection blockers, or missing branches.
7. Handoff note: merging and deployment verification happen outside this skill.
